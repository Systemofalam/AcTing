...
//--------------------------------
std::string corruptChunk(const std::string &chunk) {
    std::string corrupted = chunk;
    if (!corrupted.empty()) {
        corrupted[0] = corrupted[0] ^ 0x01; // flip one bit of the first byte
    }
    return corrupted;
}
    ...
    const std::vector<std::string> &partners)
{
    ...
    logMessage("PolluentNode " + config.nodeId + " bound to port " + std::to_string(config.port) +
               " for round " + std::to_string(roundNumber), config.logFile);

    // 2) Load owned chunks if not already loaded.
    if (isSourceNode && !loadedAlready) {
        loadedAlready = true;
        logMessage("PolluentNode " + config.nodeId + " is SOURCE. Loading data from " + config.dataFile, config.logFile);
        ...
        logMessage("PolluentNode " + config.nodeId + " loaded " + std::to_string(fileChunks.size()) + " chunks.", config.logFile);
    }

    // 3) Log current owned chunks.
    {
        std::ostringstream oss;
        oss << "PolluentNode " << config.nodeId << " owns " << ownedChunks.size()
    ...
    while (std::chrono::steady_clock::now() - roundStart < roundDuration) {
        ...
        if (!messages.empty()) {
            ...
            for (const auto &msgPair : messages) {
                ...
                // When a partner proposes a chunk that we don't have, request it.
                ...
                // When a partner sends a Pull request, respond by pushing a CORRUPTED version of the chunk.
                else if (msg.find("Header:Pull") != std::string::npos) {
                    std::string seq = extractField(msg, "Seq");
                    if (!seq.empty() && (ownedChunks.find(seq) != ownedChunks.end())) {
                        // Corrupt the chunk while preserving its length.
                        std::string corruptedData = corruptChunk(chunkStorage[seq]);
                        std::string pushMsg = "Header:Push|Seq:" + seq + "|Raw:" + corruptedData;
                        sendMessageWithLog(sockFd, pushMsg, senderAddr, config.logFile, config.nodeId);
                        logMessage("Round " + std::to_string(roundNumber) +
                                   ": Sent CORRUPTED PUSH for chunk " + seq + " to " + partnerId, config.logFile);
                    }
                }
                // Process incoming Push messages from others normally.
    ...
    {
        std::ostringstream oss;
        oss << "PolluentNode " << config.nodeId << " final ownership after round " << roundNumber << ": [";
        bool first = true;
        for (const auto &cid : ownedChunks) {
            if (!first) oss << ", ";
            first = false;
    ...
    if (!convergedLogged && ownedChunks.size() >= 20) {
        ...
        std::string expectedHash = "TestHash"; // Replace with your expected hash or config value.
    ...
    // 8) Close the socket.
    close(sockFd);
    logMessage("PolluentNode " + config.nodeId + " finished polluting round " +
