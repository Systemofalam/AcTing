    ...
    const std::vector<std::string> &partners)
{
    ...
    logMessage("Free rider node " + config.nodeId + " bound to port " + std::to_string(config.port) +
               " for round " + std::to_string(roundNumber), config.logFile);

    // Free‑rider does not load or propose any chunks.

    // Define a set of arbitrary (weird) chunk IDs to request.
    std::vector<std::string> weirdChunks = {"279", "123", "777", "42"};

    // For each partner, send pull messages for each arbitrary chunk.
    for (const auto &pid : partners) {
        ...
        for (const auto &chunkId : weirdChunks) {
            std::string pullMsg = "Header:Pull|Seq:" + chunkId;
            sendMessageWithLog(sockFd, pullMsg, partnerAddr, config.logFile, config.nodeId);
            logMessage("Round " + std::to_string(roundNumber) +
                       ": Free rider sent PULL for chunk " + chunkId + " to " + pid, config.logFile);
        }
    }

    // Set up the round duration (e.g., 2000 ms).
    auto roundStart = std::chrono::steady_clock::now();
    const auto roundDuration = std::chrono::milliseconds(2000);

    // Event loop: listen for any incoming messages during the round.
    while (std::chrono::steady_clock::now() - roundStart < roundDuration) {
        ...
        if (!messages.empty()) {
            for (const auto &msgPair : messages) {
                ...
                logMessage("Round " + std::to_string(roundNumber) +
                           " free rider received from " + partnerId + ": " + msg, config.logFile);
            }
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Close the socket at the end of the round.
    close(sockFd);
    logMessage("Free rider node " + config.nodeId + " finished round " +
    ...
    try {
        ...
        while (std::chrono::steady_clock::now() - simulationStart < simulationDuration) {
            ...
            if (roundNumber % 11 != 0) {
                ...
                freeRiderLoopOneRound(roundNumber, config, nodeDatabase, currentPartners);
